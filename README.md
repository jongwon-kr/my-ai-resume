...
| **현재 상태**              | MVP + 한국 시장 맞춤 + 포트폴리오·RAG + 챗봇 답변 품질 + 디자인 커스텀까지 구현, 프로덕션 배포 가능 수준 |
...
공개 주소·링크 복사·공개 토글은 **모든 탭에서 보이도록 상단에 고정**했습니다. 그 아래 **한눈에 보기** 위젯이 이력서 완성도, AI 답변 준비율, 최근 방문자 대화, 답변하지 못한 질문을 한 번에 보여 주고 각 타일에서 해당 탭으로 바로 이동합니다.

![대시보드 프로필 관리](docs/screenshots/08-dashboard-profile.png)

### 대시보드 — 공개 프로필 디자인

테마 프리셋 5종, 포인트 색상, 글꼴, 프로필 사진 모양, 카드·헤더 스타일을 고르면 **오른쪽 미리보기에 즉시** 반영됩니다. 미리보기는 목업이 아니라 실제 공개 화면을 그대로 렌더한 것이고, **전체 화면에서 편집**을 누르면 실제 폭 그대로 보면서 수정할 수 있습니다.
