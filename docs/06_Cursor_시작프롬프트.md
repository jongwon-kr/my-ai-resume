...
5. 폴더 구조를 다음과 같이 잡아줘:
   ...
   - app/@[id]  ← 동적 라우트로 공개 프로필 페이지 (실제로는 app/[id] 형태로 만들고 slug 앞에 @는 미들웨어에서 처리)
...
```
아래 문서들을 참고해서 공개 프로필 페이지와 실시간 챗봇을 구현해줘.
...
2. 공개 프로필 페이지에 소셜 공유 버튼 추가 (카카오톡 공유 SDK, 링크 복사, X 공유)
...
요구사항:
1. 공개 프로필 페이지에 신고하기 버튼 추가 → reports 테이블에 저장하는 API 구현
